...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    ac = actor_critic(np.zeros(obs_dim), env.action_space, **ac_kwargs)
    ac_targ = deepcopy(ac)
...
class SawyerMultiGoalPickPlaceEnv(SawyerReachPushPickPlaceEnv):
    def __init__(self, random_init=True, task_type='pick_place'):
        super().__init__(random_init,task_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ddpg_mul0')
